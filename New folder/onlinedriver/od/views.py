        ...
        if request.POST['opt']=='dvr':
            if driverdetail.objects.filter(email=request.POST['your-email']).exists() or request.POST['your-email']=='admin@gmail.com':
                ...
                if request.FILES['myfile']:
                    driverdetail(email=request.POST['your-email'],name=request.POST['full-name'],password=request.POST['password'],experience=request.POST['exp'],dlno=request.POST['dlno'],inride="0",inrideid="0",inridewith="NA",addharno=request.POST['addno'],address=request.POST['address'],pin=request.POST['pincode'],totalrides="0",ratings="0",profilepic=request.FILES['myfile']).save()
            ...
            if ownerdetail.objects.filter(email=request.POST['your-email']).exists() or request.POST['your-email']=='admin@gmail.com':
                ...
                if request.FILES['myfile']:
                    ownerdetail(email=request.POST['your-email'],name=request.POST['full-name'],password=request.POST['password'],experience=request.POST['exp'],dlno=request.POST['dlno'],inride="0",inrideid="0",inridewith="NA",addharno=request.POST['addno'],address=request.POST['address'],pin=request.POST['pincode'],profilepic=request.FILES['myfile']).save()
                    return redirect('od:login')
...
def ownerdashboard(request):
    if (request.session['usr'] and request.session['usrtype']=='own'):
        ...
        context={}
        if a[0].inride=="2" or a[0].inride=="1":
            context['data']=True
            print("PACCS4")
        else:
            context['data']=False
...
def confirmride(request,value=None):
    if (request.session['usr'] and request.session['usrtype']=='own'):
        ...
        ridedetail(destination=request.session['destination'],source=request.session['source'],driveremail=value,rid=idd[0].rid,owneremail=request.session['usr'],status="Pending",startdate=request.session['startdate'],enddate=request.session['enddate'],starttime=request.session['starttime'],endtime=request.session['endtime']).save()
...
def manageride(request):
    if (request.session['usr'] and request.session['usrtype']=='own'):
        ...
        if a[0].inride=="1":
            ...
            context['bol']=False
        context['data']=a[0]
        return render(request,'manageride.html',context)
    else:
        return redirect('od:login')
...
def cancelride(request):
    if (request.session['usr'] and request.session['usrtype']=='own'):
        a=ownerdetail.objects.filter(email=request.session['usr'])
        ridedetail.objects.filter(rid=a[0].inrideid).update(status="Cancelled")
        driverdetail.objects.filter(email=a[0].inridewith).update(inride="0",inrideid="0",inridewith="NA")
        ownerdetail.objects.filter(email=a[0].email).update(inride="0",inrideid="0",inridewith="NA")
        return redirect('od:ownerdashboard')
    else:
        return redirect('od:login')
